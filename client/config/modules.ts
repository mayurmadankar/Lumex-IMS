...
export const normalizePermissions = (rawPermissions: unknown): GroupPermission[] => {
  ...
  parsed.forEach((item) => {
    ...
    const permission = PERMISSION_LEVELS.includes(item?.permission as PermissionLevel)
      ? item.permission
      : "NONE";
...
export const permissionsToMap = (rawPermissions: unknown) =>
  normalizePermissions(rawPermissions).reduce<Partial<Record<ModuleKey, PermissionLevel>>>(
    (map, item) => {
...
export const permissionAllows = (
  ...
  required: Exclude<PermissionLevel, "NONE"> = "READ_ONLY",
